...
Drawable::Drawable(GLWidget277* context)
    : bufIdx(), bufPos(), bufNor(), bufCol(), bufVData(),
      idxBound(false), posBound(false), norBound(false), colBound(false),VDataBound(false),
      context(context)
{}
...
void Drawable::destroy()
{
    ...
    context->glDeleteBuffers(1,&bufVData);
}
...
void Drawable::generateVData()
{
    VDataBound = true;
    context->glGenBuffers(1,&bufVData);
}
bool Drawable::bindVData()
{
    if(VDataBound){
