...
MyGL::MyGL(QWidget *parent)
    : GLWidget277(parent),
      ...
      point(this), line(this), joint(nullptr)
{
...
//    prog_lambert.draw(geom_sphere);
    ...
    glm::mat4 model = glm::translate(glm::mat4(1.0f), glm::vec3(0,0,0)) * glm::rotate(glm::mat4(1.0f), glm::radians(0.0f), glm::vec3(0,0,1));
...
//    prog_flat.draw(joint);

//    printf("%d\n", joints.size());

    joint->draw(*this, prog_flat);
...
void MyGL::slot_loadSkeleton()
{
    ...
    // joint is the root joint
    joint = loadJSON(filepath);

    emit sig_sendJoint(joint);
}

Joint* MyGL::initWithJSON(QJsonObject jointJSON, Joint* parent){
    ...
    Joint* tmpJoint = new Joint(this);
    tmpJoint->name = name;
    tmpJoint->setText(0, name);
    tmpJoint->pos = position;
    tmpJoint->rot = rotation;
    tmpJoint->id = currID;
    tmpJoint->setText(0,QString(QString::number(tmpJoint->id) + " - " + name));
    tmpJoint->create();
    ...
    for (int i=0; i < jointChildren.size(); i++) {
        Joint* child = initWithJSON(jointChildren[i].toObject(), tmpJoint);
    }
...
Joint* MyGL::loadJSON(QString path) {
    QString val;
    QFile file;
    ...
    Joint* root = initWithJSON(jointJSON, NULL);
...
{
    geom_mesh.bindVertex(joint);

    glm::mat4 transformationMatrix[100];
    glm::mat4 bindMatrix[100];

    joint->getTransformationMatrix(transformationMatrix);
    joint->getBindMatrix(bindMatrix);

    prog_skeleton.setTransformationMatrix(transformationMatrix);
    prog_skeleton.setBindMatrix(bindMatrix);
}

void MyGL::slot_updateVertex()
{
    geom_mesh.updateVertex(joint, prog_skeleton);
}
