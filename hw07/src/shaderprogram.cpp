...
ShaderProgram::ShaderProgram(GLWidget277 *context)
    : vertShader(), fragShader(), prog(),
      ...
      unifModel(-1), unifModelInvTr(-1), unifViewProj(-1), unifColor(-1), unifTime(-1), unifRenderMode(-1),
      attrInfluences(-1), attrJointIDs(-1), context(context)
{}
...
void ShaderProgram::create(const char *vertfile, const char *fragfile)
{
    ...
    attrInfluences = context->glGetAttribLocation(prog, "weights");
    attrJointIDs = context->glGetAttribLocation(prog, "joint_IDs");
...
//    printf("%d\n", unifTime);
    ...
    for (int i=0; i<100; i++) {
        unifTransMats[i] = context->glGetUniformLocation(prog, QString("u_TransMats[" + QString::number(i) + "]").toStdString().c_str());
        unifBindMats[i] = context->glGetUniformLocation(prog, QString("u_BindMats[" + QString::number(i) + "]").toStdString().c_str());
    }
}
...
void ShaderProgram::setTransformationMatrix(const glm::mat4 transMats[100])
{
    useMe();

    for (int i =0; i<100; i++) {
        if (unifTransMats[i] != -1) {
            context->glUniformMatrix4fv(unifTransMats[i], 1, GL_FALSE, &transMats[i][0][0]);
        }
    }
}

void ShaderProgram::setBindMatrix(const glm::mat4 bindMats[100])
{
    useMe();

    for (int i =0; i<100; i++) {
        if (unifBindMats[i] != -1) {
            context->glUniformMatrix4fv(unifBindMats[i], 1, GL_FALSE, &bindMats[i][0][0]);
        }
    }
}
...
void ShaderProgram::setModelMatrix(const glm::mat4 &model)
{
    ...
    //context->glUniform1i(unifTime, 1);
    if (unifModelInvTr != -1) {
...
void ShaderProgram::draw(Drawable &d)
{
    ...
    if (attrInfluences != -1 && d.bindInfluences()) {
        context->glEnableVertexAttribArray(attrInfluences);
        context->glVertexAttribPointer(attrInfluences, 2, GL_FLOAT, false, 0, NULL);
    }

    if (attrJointIDs != -1 && d.bindJointIDs()) {
        context->glEnableVertexAttribArray(attrJointIDs);
        context->glVertexAttribIPointer(attrJointIDs, 2, GL_INT, 0, NULL);
    }
    ...
    if (attrInfluences != -1) context->glDisableVertexAttribArray(attrInfluences);
