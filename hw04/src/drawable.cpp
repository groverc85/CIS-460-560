...
Drawable::Drawable(GLWidget277* context)
    : bufIdx(), bufPos(), bufNor(), bufCol(), bufUvs(),
      idxBound(false), posBound(false), norBound(false), colBound(false), uvsBound(false),
      context(context)
{}
...
void Drawable::destroy()
{
    ...
    context->glDeleteBuffers(1, &bufUvs);
}
...
bool Drawable::bindUvs()
{
    if(uvsBound){
        context->glBindBuffer(GL_ARRAY_BUFFER, bufUvs);
    }
    return uvsBound;
}
